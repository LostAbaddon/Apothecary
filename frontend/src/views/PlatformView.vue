<template>
  <div class="panel" style="max-width:900px; margin:24px auto;">
    <h2><span class="section-icon">🌌</span> 擎天殿 · 玄门功法</h2>
    <div class="grid">
...
function fail(){
  ...
  selected.value.level = Math.max(1, (selected.value.level|0) - 1);
  selected.value.atk = Math.max(1, (selected.value.atk|0) - 2);
  selected.value.def = Math.max(1, (selected.value.def|0) - 2);
  alert('参悟失败，弟子走火入魔并受罚（占位）');
  cancel();
}
</script>
